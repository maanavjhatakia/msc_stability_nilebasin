...
from model_nile_scenario import ModelNileScenario
...
if __name__ == "__main__":
    ...
    #added different path to help with file distribution
    file_path_storage_joined = os.path.join(current_dir,"../outputs","")
    
    output_directory = current_dir
    output_directory_storage = file_path_storage_joined #output directory for outputs path
    ...
    n_scenarios = 5000
    policy_df = pd.read_csv(f"{output_directory_storage}policies_exploration.csv")
    my_policies = [
    ...
    with open(f"{output_directory_storage}time_counter_open_exp.txt", "w") as f:
        f.write(
    ...
    experiments.to_csv(f"{output_directory_storage}experiments_exploration.csv")
    outcomes.to_csv(f"{output_directory_storage}outcomes_exploration.csv")
